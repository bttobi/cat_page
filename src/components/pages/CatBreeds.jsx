...
const CatBreeds = () => {
  ...
  return (
      <motion.div id="cat-breeds" initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} className="home-page w-full h-full m-none mt-16  flex flex-col justify-start items-center font-article text-white">
        ...
        <button className="breed-details-button btn btn-sm bg-primary text-article text-secondary-white text-sm border-2 border-secondary-white rounded-md transition-all duration-300 hover:border-secondary-white hover:bg-bg-primary mt-4" onClick={()=>{setShowDetails(true)}}>{"show details"}</button>
        <AnimatePresence >
        ...
        <div className="cat-cards-wrapper w-full h-full flex flex-row flex-wrap items-start content-start justify-center">
          {(!query.isFetching) && query.data.map(el => {return <CatCard showClicked={getShowClicked} cat={el} key={el.url}/>})}
          {(query.isFetching) &&
