...
const Register = () => {
    ...
    try {
      ...
      switch(error.code){
        ...
        case "auth/weak-password":
          setNotificationMessage("Password should be at least 6 characters in length!");
          break;
  ...
  return (
    <motion.div initial={{scaleY: 0}} animate={{scaleY: 1}} exit={{scaleY: 0}} className="flex flex-col justify-center align-center items-center font-article text-white w-32 h-min mt-40">
      <AnimatePresence>
        {showNotification && <Notification notification={ notificationMessage } errorHappened={errorHappened} icon={notificationIcon}/>}
      </AnimatePresence>
