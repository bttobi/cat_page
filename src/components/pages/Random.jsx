...
const Random = () => {
  ...
  const fetchData = async () => {
    const URL = `https://api.thecatapi.com/v1/images/search?limit=16&api_key=${
  ...
  return (
    <>
      <motion.div
        ...
        animate={{ scaleY: 1 }}
        exit={{ scaleY: 0 }}
        className="home-page w-full my-16 flex flex-col justify-start items-center font-article text-white"
      ...
      >
        ...
        {query.isFetching && (
          <div className="fixed top-0 bottom-0 rounded-lg loading-wrapper m-16 flex flex-col self-center items-center justify-center">
            ...
            <span className="loading-text font-bold font-article text-white">
              Loading...
            </span>
          </div>
        )}
      ...
      <AnimatePresence>
        {query.isError && (
          <Notification
            notification={"Failed while fetching normal cats!"}
            errorHappened={query.isError}
          />
        )}
      </AnimatePresence>
    </>
