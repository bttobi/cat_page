...
const Favourites = () => {
  ...
  if (!isFetched) {
    //if not yet fetched check if user is logged in
  ...
  const filterCats = () => {
    setFilteredCats(
      cats.filter((cat) => {
        ...
        return (
          (customName ?? breedName ?? "cute cat").includes(
            searchRef.current.value.toLowerCase()
          ) ||
          (breedName ?? "cute cat").includes(
            searchRef.current.value.toLowerCase()
          )
        );
      })
  ...
  return (
    <>
      <motion.div
        ...
        animate={{ scaleY: 1 }}
        exit={{ scaleY: 0 }}
        className="favourites-wrapper w-full h-full m-none my-32 flex flex-col justify-center items-center font-article text-white"
      >
            ...
            <AnimatePresence>
              {showClicked && (
                <motion.div
                  ...
                  animate={{ top: 0, opacity: 0.7 }}
                  exit={{ opacity: 0 }}
                  className="backdrop w-full h-full fixed flex flex-col bg-black z-10 align-center justify-center items-center"
                ></motion.div>
              )}
            </AnimatePresence>
            {filteredCats != [] && (
            ...
            <div className="cat-cards-wrapper w-full h-full flex flex-wrap items-start align-start content-start justify-center">
              {!isFetched && filteredCats.length != [] ? (
                <div className="loading-wrapper z-10 m-16 flex flex-col items-center justify-center">
                  ...
                  <span className="loading-text text-base font-bold font-article text-white">
                    Loading...
                  </span>
                </div>
                ...
                <div className="not-loggedin-notification font-bold text-xl underline mt-4">
                  <Link to="/random">
                    No favourite cats found... Add some to favourites!
                  </Link>
                </div>
              )}
            </div>
        ...
        {errorHappened && (
          <Notification
            notification={
              "Error has happened while fetching! Will try again in 5 seconds..."
            }
