...
const CatCard = (props) => {
  ...
  const handleFavourites = () => {
    ...
    try {
      if (!isFavourite) {
        //you can only add if not favourite
  ...
  const hideDetails = (e, clicked) => {
    if (
      catDetails.current != null &&
      (!catDetails.current.contains(e.target) || clicked)
    ) {
      setIsShown(false);
    ...
    if (
      (inputChangeNameRef.current != null &&
        (!inputChangeNameRef.current.contains(e.target) || clicked) &&
        !editNameRef.current.contains(e.target)) ||
      clicked
    ) {
      setShowEditName(false);
    }
    ...
    <>
      <AnimatePresence>
        <motion.div
          ...
          className="cat-wrapper w-min h-min mx-4 mt-8 flex flex-col bg-primary rounded-lg shadow-lg shadow-black"
        >
          <div className="favourite py-6 pr-2 pl-2 w-full h-14 flex flex-row justify-center items-center align-center">
            <button
              className="fav-button absolute ml-2 left-0 align-start w-min h-min transition-all duration-200 hover:scale-125"
              onClick={() => {
                  ...
                  {showEditName && isFavourite ? (
                    ...
                    >
                      {props.cat?.customName
                        ? props.cat?.customName
                        : props.cat?.breeds[0]?.name ?? "Cute Cat"}
                    </motion.span>
                  )}
                </AnimatePresence>
              </p>
            ...
            <button
              className="fav-button absolute right-0 mr-2 align-end w-min h-min transition-all duration-200 hover:scale-125"
          ...
          >
            <div
              className="w-min h-min m-0 rounded-lg"
              style={{
                minWidth: "12rem",
                minHeight: "14rem",
                backgroundImage: `url(${props.cat.url})`,
      ...
      <AnimatePresence>
        {isShown && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="fixed top-28 flex flex-col justify-center align-center items-center z-10 filter-blur-0"
            ref={catDetails}
          >
            <CatCardClicked
              cat={props.cat}
              isFavourite={isFavourite}
              showFunc={hideDetails}
              handleFavFunc={handleFavourites}
              handleEditNameFunc={editName}
              handleRemoveNameFunc={removeName}
            />
          </motion.div>
        )}
      </AnimatePresence>
