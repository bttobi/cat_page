...
const CatCardClicked = (props) => {
  ...
  return (<>
    <div className="cat-details m-auto z-20 flex flex-col rounded-lg transition-all duration-300 opacity-100 font-article" style={{minWidth:"90vw", maxWidth: "90vw"}}>
      <div className="flex flex-col flex-grow cat-clicked-wrapper p-2 bg-primary rounded-lg shadow-lg shadow-black justify-start align-start items-center"  style={{minHeight: "50vh", background: "rgba(0, 0, 0, .65) " + `url(${props.cat.url})`, backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat: "no-repeat", backgroundBlendMode: "darken"}}>
        ...
        <div className="flex flex-col items-center justify-center align-center h-full">
          {showEditName ? 
            <motion.input initial={{scaleX: 0}} animate={{scaleX: 1}} exit={{scaleX: 0}} ref={inputChangeNameRef} onKeyDown={handleKeys} className="input lg:input-lg mt-4 lg:mt-6" placeholder="Edit name"/> 
          ...
          <div className="cat-description-wrapper mt-4 lg:mt-8">
            ...
            <div className="w-full flex flex-col justify-center items-center">
              {props.cat.customName != null && <div className="font-bold text-lg" style={{fontSize: "clamp(1rem, 3vw, 2.5rem)"}}>{"Race: " + (props.cat?.breeds[0]?.name)}</div>}
        ...
        <div className="button-wrapper h-full flex flex-col justify-center align-center w-full items-center content-center lg:mb-8 mb-0 lg:mt-4">
            {!showEditName && <button className="btn btn-sm align-end p-2 h-full transition-all duration-200 hover:scale-110 bg-dark border-none rounded-lg mt-4" style={{fontSize: "clamp(0.8rem, 1.5vw, 1.8rem)"}} onClick={props.handleFavFunc}>{props.isFavourite ? "🗑️ REMOVE FROM FAVOURITES" : "❤️ ADD TO FAVOURITES"}</button>}
            {props.isFavourite == true && <button className="btn btn-sm align-end p-2 h-full transition-all duration-200 hover:scale-110 bg-dark border-none rounded-lg mt-4" style={{fontSize: "clamp(0.8rem, 1.5vw, 1.5rem)"}} onClick={()=>{setShowEditName(!showEditName)}}>{showEditName ? <span onClick={()=>{props.handleEditNameFunc(inputChangeNameRef?.current?.value)}}>✅ SET NEW NAME</span> : "✏️ EDIT NAME"}</button>}
